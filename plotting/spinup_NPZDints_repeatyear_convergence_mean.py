"""
Test whether mean stabilizes in successive runs by plotting mean values
...
for region in regions:
    ...
    ##########################################################
    ##  Plot averages of successive spin-up runs    ##
    ##########################################################
...
for region in regions:
    for variable_info in plot_variables:
        ...
        # ------------------------------------------------------
        # Calculate average value for each repeated-year run
        # ------------------------------------------------------
        ...
        for i, gtagex in enumerate(gtagexes):
            ...
            if gtagex not in rerun_data:
                continue

            data = rerun_data[gtagex]
        ...
        print(f'\nAverage {var_name} for each repeated-year run:')

        for run, mean_val in zip(run_numbers, mean_values):

            print(
                f'Run {run}: '
                f'average {var_name} = {mean_val:.4f}'
            )
        ...
        ax.set_title(
            f'{region} – {var_name} repeated-year simulations',
            loc='left',
            fontsize=14,
            fontweight='bold'
        )
        ...
        print(f'Plot saved to {out_dir / output_filename}')
