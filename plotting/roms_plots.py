...
def P_basic_oly(in_dict):
    # Like P_basic but optimized for the oly1 grid
    # START
    ds = xr.open_dataset(in_dict['fn'])
    # find aspect ratio of the map
    aa = pfun.get_aa(ds)
    ...
    fs = 14
    hgt = 8
    pfun.start_plot(fs=fs, figsize=(hgt*2.2/AR,hgt))
    fig = plt.figure()
    # PLOT CODE
    vn_list = ['salt', 'temp']
    ii = 1
    for vn in vn_list:
        ...
        if ii == 1:
            ...
            ax.set_yticklabels([])
            # pfun.add_velocity_vectors(ax, ds, in_dict['fn'],v_scl=20)
        ii += 1
    fig.tight_layout()
    # FINISH
    ds.close()
