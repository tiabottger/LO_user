...
Tia edited this version to plot model mean of SalishSeaCast, 
        ...
        for depth_range in depth_range_list:
            for time_range in time_range_list:
                ...
                for ii in range(len(vn_list)):
                    ...
                    # Calculate ensemble mean of model values
                    
                    model_values = np.column_stack(model_values)
                    
                    # Mean across available models for each observation.
                    y_mean = np.nanmean(model_values, axis=1)
                            
                    # Keep only paired finite observation-model values
                    valid = np.isfinite(x) & np.isfinite(y_mean)
                    x_valid = x[valid]
                    y_valid = y_mean[valid]
                        
                    # Plot model mean vs observations    
                    ax.plot(x_valid,y_valid,marker='.',ls='',color='k', alpha=alpha)
                    ...
                    ax.text(.95,
                            t_dict[gtx],
                            'bias=%0.1f, rmse=%0.1f' % (bias,rmse),c='k',
                    ...
                    if otype == 'bottle':
                        ...
                        if jj in [1,5,9]:
                            ax.set_ylabel('Model mean', fontsize=16)
                    elif otype == 'ctd':
                        if jj in [4,5]:
                ...
                if testing:
                    ...
                    plt.savefig(out_dir / (ff_str + '_ensemble.png'))
                    plt.close('all')
